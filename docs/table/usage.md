...
- 表格容器启用 `overflow-x: auto`，可通过左右滑动查看超出的列。
- 表头与单元格内容强制 `white-space: nowrap`，避免内容换行导致列宽被压缩、行高不一致。
- 列宽仍按 `width` / `minWidth` 计算，建议为关键列设置合适的 `minWidth`，保证横向滚动时内容可读。

> 本次适配未新增 props，保持现有 `data` / `columns` 用法不变。
> 注意：如果表格外层存在 `w-dialog`，移动端弹窗会自动全屏展示，可在全屏状态下横向滚动查看完整表格。
