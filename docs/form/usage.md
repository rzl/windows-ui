...
| validator | 自定义验证函数，支持同步返回或返回 Promise | `{ validator: (val) => val > 0 || '必须大于0' }` |
| asyncValidator | 异步验证函数，返回 Promise<string \| undefined> | `{ asyncValidator: async (val) => { const ok = await checkName(val); if (!ok) return '名称已存在' } }` |
