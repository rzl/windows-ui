...
- 悬停或选中节点时显示节点工具栏，包含：拖动排序、添加子组件、上移、下移、删除，均使用图标按钮。
- 选中节点后按 `Delete` / `Backspace` 可快速删除，`Esc` 取消选中。
- 从左侧组件库拖拽组件到画布或容器时，根画布与容器区域会高亮提示；拖入容器后会自动选中子组件并记录历史。
- 拖拽节点工具栏的「拖动排序」图标，可将节点在同级列表中排序；拖拽到目标节点上方/下方/容器内部时会显示对应高亮提示，松开后自动记录历史。
- 工具栏提供撤销、重做、复制、粘贴、缩放、网格、深色模式、预览、保存等图标操作。

## 撤销/重做

- 属性编辑（输入框、选择器、颜色、JSON 文本等）采用 `300ms` 防抖记录历史，避免连续输入产生大量撤销点。
- 添加、删除、移动、排序、粘贴等即时操作会立即刷新待定的属性变更，再记录新的历史状态，确保撤销顺序正确。
- 支持 `Ctrl+Z` / `Ctrl+Shift+Z` / `Ctrl+Y` 快捷键。

## 大纲联动

- 左侧「大纲」面板展示页面组件层级，点击节点可快速选中画布对应组件。
- 当选中嵌套在容器内的组件时，大纲会自动展开该组件的所有祖先容器，确保选中节点可见。
- 大纲节点同样支持拖拽排序，拖动节点到目标节点上方/下方/容器内部可调整层级与顺序。
