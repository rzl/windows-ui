<template>
  <div class="model-version-panel">
    <div class="toolbar">
      <w-button v-if="isAdmin" type="primary" size="small" @click="openCreateDialog">+ 创建快照</w-button>
...
import * as modelVersionApi from '@/api/model-version'
...
async function loadVersions() {
  const res = await modelVersionApi.getModelVersions(props.modelId)
...
async function handleCreate() {
  await modelVersionApi.createModelVersion(props.modelId, {
...
async function handleRollback(row: any) {
  if (!confirm(`确定要回滚到版本 ${row.version} 吗？当前配置将被覆盖。`)) return
  await modelVersionApi.rollbackModelVersion(props.modelId, row.id)
  emit('rollback')
  await loadVersions()
}
...
async function handleDelete(row: any) {
  ...
  await modelVersionApi.deleteModelVersion(props.modelId, row.id)
  await loadVersions()
}
