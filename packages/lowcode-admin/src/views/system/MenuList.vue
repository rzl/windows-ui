<template>
  <div class="list-page">
    <w-card header="菜单管理">
      ...
      <w-table :data="flatMenus" :columns="columns" stripe border row-key="id">
...
const flatMenus = computed(() => {
  const result: any[] = []
  const map = new Map()
  menus.value.forEach((m) => map.set(m.id, m))

  function flatten(items: any[], parentTitle = '') {
    items.forEach((item) => {
...
const parentOptions = computed(() => {
  ...
  menus.value.forEach((m) => {
    result.push({ label: m.title, value: m.id })
  })
  return result
})
