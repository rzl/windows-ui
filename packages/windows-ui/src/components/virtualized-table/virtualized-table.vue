<template>
  <div class="w-virtualized-table">
    <div class="w-virtualized-table__header"><div v-for="col in columns" :key="col.prop" class="w-virtualized-table__th">{{ col.label }}</div></div>
    <div class="w-virtualized-table__viewport" :style="{ height: `${rowHeight * visibleCount}px` }" @scroll="handleScroll">
      <div class="w-virtualized-table__body" :style="{ height: `${data.length * rowHeight}px` }">
        <div v-for="(row, ri) in visibleData" :key="startIndex + ri" class="w-virtualized-table__row" :style="{ height: `${rowHeight}px`, top: `${(startIndex + ri) * rowHeight}px` }">
          <div v-for="col in columns" :key="col.prop" class="w-virtualized-table__td"><slot :name="col.prop" :row="row" :$index="startIndex + ri">{{ row[col.prop] }}</slot></div>
        </div>
      </div>
    </div>
  </div>
...
const props = defineProps({ data: { type: Array as () => any[], default: () => [] }, columns: { type: Array as () => { prop: string; label: string }[], default: () => [] }, rowHeight: { type: Number, default: 32 }, visibleCount: { type: Number, default: 10 } })
