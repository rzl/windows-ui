  ...
  <div class="w-tree-select" v-click-outside="close">
    <div class="w-tree-select__trigger" @click="open = !open">
      ...
      <div class="w-tree-select__icons">
        <w-icon v-if="clearable && selectedLabel" name="close" size="small" class="w-tree-select__clear" @click.stop="handleClear" />
...
const props = defineProps({ modelValue: [String, Number] as any, data: { type: Array as () => any[], default: () => [] }, placeholder: { type: String, default: '请选择' }, expandAll: Boolean, clearable: { type: Boolean, default: true } })
