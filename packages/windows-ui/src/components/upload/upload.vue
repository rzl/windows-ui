<template>
  <div :class="['w-upload', `w-upload--${size}`]">
    <div class="w-upload__trigger" @click="handleClick">
      ...
      <slot><w-button :size="size">{{ buttonText }}</w-button></slot>
    </div>
    <div v-if="fileList.length" class="w-upload__list">
      <div v-for="(file, i) in fileList" :key="i" class="w-upload__item"><w-icon name="file" :size="size" /><span class="w-upload__name">{{ file.name }}</span><span class="w-upload__size">({{ formatSize(file.size) }})</span><w-icon name="close" :size="size" class="w-upload__remove" @click="remove(i)" /></div>
    </div>
  </div>
</template>
...
const props = defineProps({ accept: String, multiple: Boolean, buttonText: { type: String, default: '选择文件' }, size: { type: String, default: undefined } })
