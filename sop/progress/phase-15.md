...
| `packages/lowcode-admin/src/stores/app.ts` | 新增 `locale`，主题/尺寸/语言持久化到 `localStorage` |
| `packages/lowcode-admin/src/App.vue` | `w-config-provider` 增加 `locale` 绑定 |
...
7. [x] `pnpm build:lowcode` 无类型错误，构建成功。
